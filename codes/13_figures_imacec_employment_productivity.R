...
# Fecha trimestral: primer día del último mes del trimestre (mar/jun/sep/dic)
...
# ---------------------------------------------------------------
# 1) Estandarizar tipos (SIN if_else dentro de mutate)
# ---------------------------------------------------------------
...
# Si existe t_index en productividad, lo convertimos; si no, no pasa nada
if ("t_index" %in% names(prod_total)) prod_total <- prod_total %>% dplyr::mutate(t_index = as.numeric(t_index))
if ("t_index" %in% names(prod_nm))    prod_nm    <- prod_nm    %>% dplyr::mutate(t_index = as.numeric(t_index))

# ---------------------------------------------------------------
# 2) Checks mínimos (para fallar informando)
# ---------------------------------------------------------------
stopifnot(all(c("year","trimestre","imacec_indice") %in% names(imacec_tri)))
stopifnot(all(c("year","trimestre","empleo_indice") %in% names(empleo_tri)))
stopifnot(all(c("year","trimestre","prod_indice") %in% names(prod_total)))
stopifnot(all(c("year","trimestre","prod_indice") %in% names(prod_nm)))

# ---------------------------------------------------------------
# 3) Productividad total
# ---------------------------------------------------------------
df_prod_total <- prod_total %>%
  dplyr::mutate(fecha_tri = to_fecha_tri(year, trimestre)) %>%
  dplyr::filter(!is.na(fecha_tri), !is.na(prod_indice)) %>%
  dplyr::arrange(fecha_tri)

p_prod_total <- ggplot2::ggplot(df_prod_total, ggplot2::aes(fecha_tri, prod_indice)) +
...
p_prod_nm <- ggplot2::ggplot(df_prod_nm, ggplot2::aes(fecha_tri, prod_indice)) +
  ggplot2::geom_line(linewidth = 0.9, na.rm = TRUE) +
  ggplot2::scale_x_date(date_breaks = "2 years", date_labels = "%Y") +
  ggplot2::labs(
...
# ---------------------------------------------------------------
# 5) Productividad total vs no minera (comparado)
# ---------------------------------------------------------------
prod_long <- dplyr::bind_rows(
  df_prod_total %>% dplyr::transmute(fecha_tri, serie = "Total", indice = prod_indice),
  df_prod_nm    %>% dplyr::transmute(fecha_tri, serie = "No minera", indice = prod_indice)
) %>%
  dplyr::arrange(fecha_tri)
...
# ---------------------------------------------------------------
# 6) IMACEC total vs empleo (comparado)
# ---------------------------------------------------------------
